...
import { useFormManager, FormInputData, EFormInputType, IFormInitalState, IFormInputAvailableValue } from 'vdr-react-form-manager';
import { formClasses, h2Classes, containerClasses } from '../constant/App.constant';

const formInitalState = {
	formInputs: {
		...FormInputData.Builder(EFormInputType.INPUT_TYPE_RADIO, 'myradiobox')
...
export function InputRadioComponent() {
	...
	function renderRadio({ value, label } : IFormInputAvailableValue) {
		return <label key={value} className="inline-block pr-3">{label}&nbsp;<input type="radio" value={value} name='myradiobox'/></label>
	}
